...
//helper function to remove write perm from pte
int
mprotect_helper(pde_t *pgdir, void *va, int len) {
	pte_t *pte; 
	for (int i = 0; i < len; i++) {
	...
	lcr3(PADDR(pgdir)); // reload cr3 register holding addr
	return 0;
}

//helper function to add back write perm
int 
munprotect_helper(pde_t *pgdir, void *va, int len) {
	pte_t *pte;
	for (int i = 0; i < len; i++) {
		pte = walkpgdir(pgdir, va, 0);
		if (pte == 0) {
